...
import sys
# https://github.com/kwanlowe/gce-ansible-inventory.git
import json
import optparse
import ConfigParser
...
# Update the following line to the location of your inventory config file.
...
def readGceConfig(gceConfigFile, nodeName):
    ...
    try:
        ...
        print("Unable to locate node config file ", gceConfigFile, '.\n')
        sys.exit(3)
        raise
    except ConfigParser.NoSectionError:
        print("Unable to locate section [", nodeName, "]" + " in ",
              gceConfigFile, ".\n")
        sys.exit(3)
    else:
...
parser.add_option('--host',
                  action="store",
                  dest="hostname",
                  help="Return host list")
options, args = parser.parse_args()

if (options.invlist is None) and (options.hostname is None):
